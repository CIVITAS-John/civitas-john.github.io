...
function thresholdFreedmanDiaconis(values, min, max) {
  ...
  return c && d ? Math.ceil((max - min) / (2 * d * c**(-1 / 3))) : 1;
}
...
function axis(orient, scale) {
  ...
  function axis(context) {
    ...
    tick
        .attr("opacity", 1)
        .attr("transform", (d) => { return transform(position(d) + offset); });
...
function rgb2lrgb(x) {
  return (x /= 255) <= 0.04045 ? x / 12.92 : ((x + 0.055) / 1.055)**2.4;
}
...
function exponential$1(a, b, y) {
  return a = a**y, b = b**y - a, y = 1 / y, function(t) {
    return (a + t * b)**y;
  };
}
...
function brush$1(dim) {
  ...
  function brush(group) {
    ...
    var handle = group.selectAll(".handle")
      .data(dim.handles, (d) => { return d.type; });

    handle.exit().remove();

    handle.enter().append("rect")
        .attr("class", (d) => { return `handle handle--${  d.type}`; })
        .attr("cursor", (d) => { return cursors[d.type]; });
  ...
  function redraw() {
    ...
    if (selection) {
      ...
      group.selectAll(".handle")
          .style("display", null)
          .attr("x", (d) => { return d.type[d.type.length - 1] === "e" ? selection[1][0] - handleSize / 2 : selection[0][0] - handleSize / 2; })
          .attr("y", (d) => { return d.type[0] === "s" ? selection[1][1] - handleSize / 2 : selection[0][1] - handleSize / 2; })
          .attr("width", (d) => { return d.type === "n" || d.type === "s" ? selection[1][0] - selection[0][0] + handleSize : handleSize; })
          .attr("height", (d) => { return d.type === "e" || d.type === "w" ? selection[1][1] - selection[0][1] + handleSize : handleSize; });
    }
...
function dsvParse(parse) {
  return function(input, init, row) {
    ...
    return text(input, init).then((response) => {
...
function graticuleX(y0, y1, dy) {
  ...
  return function(x) { return y.map((y) => { return [x, y]; }); };
}

function graticuleY(x0, x1, dx) {
  var x = range$2(x0, x1 - epsilon$1, dx).concat(x1);
  return function(y) { return x.map((x) => { return [x, y]; }); };
}
...
function graticule() {
  ...
  function lines() {
    return range$2(ceil(X0 / DX) * DX, X1, DX).map(X)
        ...
        .concat(range$2(ceil(x0 / dx) * dx, x1, dx).filter((x) => { return abs$1(x % DX) > epsilon$1; }).map(x))
        .concat(range$2(ceil(y0 / dy) * dy, y1, dy).filter((y) => { return abs$1(y % DY) > epsilon$1; }).map(y));
  }

  graticule.lines = function() {
    return lines().map((coordinates) => { return {type: "LineString", coordinates}; });
  };
...
var azimuthalEqualAreaRaw = azimuthalRaw((cxcy) => {
...
var azimuthalEquidistantRaw = azimuthalRaw((c) => {
...
function conicConformalRaw(y0, y1) {
  ...
  project.invert = function(x, y) {
    ...
    return [l / n, 2 * atan((f / r)**(1 / n)) - halfPi$1];
  };
...
function band() {
  ...
  function rescale() {
    ...
    var values = range$2(n).map((i) => { return start + step * i; });
...
function pie() {
  ...
  function pie(data) {
    ...
    if (sortValues != null) index.sort((i, j) => { return sortValues(arcs[i], arcs[j]); });
    else if (sort != null) index.sort((i, j) => { return sort(data[i], data[j]); });
...
CatmullRom.prototype = {
  ...
  point(x, y) {
    ...
    if (this._point) {
      var x23 = this._x2 - x,
          y23 = this._y2 - y;
      this._l23_a = Math.sqrt(this._l23_2a = (x23 * x23 + y23 * y23)**this._alpha);
    }
...
function appearance(series) {
  ...
  return none(series).sort((a, b) => { return peaks[a] - peaks[b]; });
}
...
function ascending(series) {
  ...
  return none(series).sort((a, b) => { return sums[a] - sums[b]; });
}
...
function zoom() {
  ...
  function wheeled(event, ...args) {
    ...
    var g = gesture(this, args).event(event),
        t = this.__zoom,
        k = Math.max(scaleExtent[0], Math.min(scaleExtent[1], t.k * 2**wheelDelta.apply(this, arguments))),
        p = pointer(event);
